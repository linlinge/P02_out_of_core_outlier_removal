...
{
    int colon_pos=range.find(":");
    int start_idx=atoi(range.substr(0,colon_pos).c_str());
    int end_idx=atoi(range.substr(colon_pos+1,range.length()-colon_pos-1).c_str());
    vector<int> dat1;
...
void VectorWrite(string filename,vector<int>& dat,string mode)
{
    std::ofstream file;
    if (file.bad())
        std::cout << "cannot open file" << std::endl;
